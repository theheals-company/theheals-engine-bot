...
# ── 장기작업 하트비트 (ORD-2026-0711-P2 §2): "실행중" 30분 이상 정체 시 생존신호 ──
HEARTBEAT_THRESHOLD_SECONDS = int(os.environ.get("HEARTBEAT_THRESHOLD_SECONDS", str(30 * 60)))
HEARTBEAT_CHECK_INTERVAL_MINUTES = int(os.environ.get("HEARTBEAT_CHECK_INTERVAL_MINUTES", "5"))

SYSTEM_PROMPT = """당신은 더힐즈컴퍼니(The Heals Company)의 더힐즈 엔진 PM 에이전트 '관제봇'입니다.
...
# ── 추론 티어링 (v0.2 §7-2): 일상 처리는 Sonnet 기본, 판단 필요 감지 시 Opus 자동 승격.
# 신규설계·2회실패진단·분기증류(Fable 3출격조건, governance.escalate_to_principal)는 불변 — 손대지 않음.
JUDGMENT_KEYWORDS = ("의사결정", "전략", "리스크", "왜", "판단", "위험")


def needs_judgment_tier(user_msg: str) -> bool:
    """질문에 의사결정/전략/리스크/왜 성격이 감지되면 Opus 티어로 자동 승격 대상."""
...
def _progress_channel(guild: discord.Guild | None) -> discord.abc.Messageable | None:
    if guild is None:
        return None
    return discord.utils.get(guild.text_channels, name=PROGRESS_CHANNEL)
...
# ── 중간보고 버튼 (ORD-2026-0711-P2 §1, 카파시1원칙): 실행중 실패/모호함 시
# 깨진 답을 그냥 승인게이트로 흘려보내지 않고 멈춰서 대표 판단을 구한다. ──
class MidReportView(discord.ui.View):
    def __init__(self, order_id: str):
        super().__init__(timeout=None)  # 대표 응답 대기는 무기한 — 워치독 타임아웃 대상 아님
        self.order_id = order_id

    async def _resolve(self, i: discord.Interaction, label: str, status: str):
        for c in self.children:
            c.disabled = True
            c.style = discord.ButtonStyle.secondary
        await i.response.edit_message(content=label, view=self)
        self.stop()
        orders_store.update_status(self.order_id, status)
        await post_progress(i.guild, self.order_id, f"{status} (중간보고 응답)")

    @discord.ui.button(label="계속 진행", style=discord.ButtonStyle.success, emoji="▶️")
    async def keep_going(self, i, b):
        await self._resolve(i, "▶️ **계속 진행 선택됨**", "실행중")

    @discord.ui.button(label="수정 필요", style=discord.ButtonStyle.primary, emoji="✏️")
    async def needs_revision(self, i, b):
        await self._resolve(i, "✏️ **수정 필요로 표시됨**", "수정요청")

    @discord.ui.button(label="중단", style=discord.ButtonStyle.danger, emoji="⏹️")
    async def abort(self, i, b):
        await self._resolve(i, "⏹️ **중단됨**", "반려")


async def post_mid_report(
    guild: discord.Guild | None, order_id: str, reason: str, source_message: discord.Message | None = None
) -> discord.Message | None:
    """실행중 단계에서 모호함/실패가 발생하면 #진행상황에 질문 임베드 + 3버튼 게시.
    이 파이프라인은 동기 실행이라 버튼 클릭까지 이 함수가 기다리지는 않는다 —
    응답은 orders.db 상태전이로 비동기 반영되고, on_message는 여기서 즉시 반환한다."""
    ch = _progress_channel(guild)
    if not ch:
        return None
    embed = discord.Embed(
        title=f"❓ 중간보고 — [{order_id}]",
        description=f"판단이 필요합니다: {reason}",
        color=discord.Color.orange(),
    )
    if source_message is not None:
        embed.add_field(name="↩️ 원 발주", value=f"[바로가기]({source_message.jump_url})", inline=False)
    view = MidReportView(order_id=order_id)
    try:
        msg = await ch.send(embed=embed, view=view)
    except discord.HTTPException:
        return None
    orders_store.update_status(order_id, "중간보고")
    return msg
...
async def _post_heartbeats_once() -> int:
    """'실행중'으로 HEARTBEAT_THRESHOLD_SECONDS 이상 머문 발주마다 생존신호 1줄 게시.
    봇이 죽었는지 일하는 중인지 구분 안 되는 문제의 직접 해결책(ORD-2026-0711-P2 §2)."""
    due = orders_store.list_orders_needing_heartbeat("실행중", HEARTBEAT_THRESHOLD_SECONDS)
    if not due:
        return 0
    posted = 0
    ...
    for guild in bot.guilds:
        ...
        for order in due:
            ...
            elapsed_min = int((now - entered_at).total_seconds() // 60)
            try:
                await ch.send(f"⏳ [{order['id']}] 실행 중... (경과 {elapsed_min}분)")
                posted += 1
            except discord.HTTPException:
                pass
    for order in due:
        orders_store.mark_heartbeat(order["id"])
    return posted


@tasks.loop(minutes=HEARTBEAT_CHECK_INTERVAL_MINUTES)
async def heartbeat_check():
    await _post_heartbeats_once()
